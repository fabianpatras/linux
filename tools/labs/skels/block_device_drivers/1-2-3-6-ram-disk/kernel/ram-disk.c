				     ...
				     const struct blk_mq_queue_data *bd)
{
	...
	if (blk_rq_is_passthrough(rq)) {
		printk("We are not interested in this request, sorry\n");
		blk_mq_end_request(rq, BLK_STS_IOERR);
		goto out;
	}

	/* TODO 2: print request information */
	// printk("Request received bro\n");
	// printk("Sector [%lld]\n", blk_rq_pos(rq));
	// printk("Total size to transfer [%d]\n", blk_rq_bytes(rq));
	// printk("Data size from current bio [%d]\n", blk_rq_cur_bytes(rq));
...
static int __init my_block_init(void)
{
	...
	if (err) {
		printk("[ERR] Nu am inregistrat block_device-ul pt ca [%d]\n", err);
	...
	if (err) {
		printk("[ERR] Nu am creat block_device-ul pt ca [%d]\n", err);
		goto out_create_block_device;
	}

	return 0;

out_create_block_device:
